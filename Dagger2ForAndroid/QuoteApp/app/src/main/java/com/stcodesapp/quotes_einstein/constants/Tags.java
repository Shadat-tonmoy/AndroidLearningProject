package com.stcodesapp.quotes_einstein.constants;

public class Tags {
    public static final String HOME = "HOME";
    public static final String QUOTES = "QUOTE";
    public static final String QUOTE_TEXT = "quote_text";
    public static final String QUOTES_BUNDLE_KEY = "quote_key";
    public static final String NAVIGATION_MENU_1 = "NAVIGATION_MENU_1";
    public static final String NAVIGATION_MENU_2 = "NAVIGATION_MENU_2";
    public static final String NAVIGATION_MENU_3 = "NAVIGATION_MENU_3";
}
